...
function listStaleComputerUsePids() {
  ...
  const result = spawnSync("pgrep", ["-fal", "HS Code \\(Dev\\).*(computerUseMcp\\.mjs mcp)"], {
    encoding: "utf8",
  });
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale HS Code (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function warnIfAlphaAppRunning() {
  ...
  const result = spawnSync(
    "pgrep",
    ["-fal", "/Applications/HS Code \\(Alpha\\)\\.app/Contents/MacOS/HS Code \\(Alpha\\)"],
    { encoding: "utf8" },
  );
  const output = typeof result.stdout === "string" ? result.stdout.trim() : "";
  if (!output) {
  ...
  console.error(
    "[desktop-dev] HS Code (Alpha) is still running. Close it before testing voice in HS Code (Dev), or you may be looking at the wrong app/runtime.",
  );
